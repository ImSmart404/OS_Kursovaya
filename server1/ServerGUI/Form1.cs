...
namespace server1
{
    public partial class Form1 : Form
    {
        ...
        {
                InitializeComponent();
                server = new SocketServer("127.0.0.1", 9000);
                server.Start();
                ...
                try
                {
                    using (Socket client = this.server.Accept())   //новый сокет для клиента
                    {
                        ...
                        if (client.Connected)                                                                      //подключились ли мы к сокету?
                        {
                            ...
                            int numberOfScreens = SystemInformation.MonitorCount;
                            message.Add("Кол-во мониторов: " + numberOfScreens);
                            message.Add("Разрешение монитора: " + "Height:" + screenHeight + " Width:" + screenWidth);
                            message.Add("[end]");
